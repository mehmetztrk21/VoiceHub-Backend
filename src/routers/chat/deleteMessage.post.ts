...
export default async function ({ body, voiceHubDb, req, session }: AppContext<Request>) {
    ...
    if (message.senderId.toString() != user._id.toString()) return response.setError("Unauthorized");
    await mongoDb.collection("messages").updateOne({ _id: new ObjectId(body.messageId) }, { $set: { isDeleted: true } });
    return response.setSuccess("Message deleted");
} 
